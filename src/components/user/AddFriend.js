    ...
    const getUsers = () => {
        getAllUsers()
            ...
            .then((applicableUsers) => {
                // creates empty array, loops through all non logged in users, then loops through all users who are already on the friends list - if a user object from the list of all non-logged in users has an id that doesn't match any users on the alreadyFriends objects, the non-loggged in user object is pushed into notFriends; state of "users" is declared as the user objects in the notFriends
                const notFriends = []
                for (let i = 0; i < applicableUsers.length; i++) {
                    ...
                    for (let n = 0; n < alreadyFriends.length; n++) {
                        ...
                        if (friendObj.userId !== applicableUserObj.id) {
                            notFriends.push(applicableUserObj)
                        }
                    }
                }
                setUsers(notFriends)
